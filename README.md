...
- **Pipeline** — leads through Job Sold and lost. Every new estimate opens a Lead if it is not already on the board. When the homeowner signs, the card moves to Job Sold and a precon job is created. Each card shows a job code (`BJ081926-A`) assigned when the lead is opened. **New lead** slides in from the right: assignee, homeowner name, phone, email, job-site address, and how they heard about you. If the source is Referral, search contacts this seat can see and connect the referrer.
...
- **Estimates** — Joist-style writer: sections, optional lines, tax, discount, deposit, terms, and a client preview. Download a PDF, send a copyable client link, mark it signed (Job Sold + a job), convert included lines to an invoice. EST-1001–1010 against homeowners.
