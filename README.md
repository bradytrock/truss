...
- **Price book** — company catalog of labor, material, equipment, allowances, and subcontract packages. Add, edit, and remove items under **Price book**. Estimators drop them onto a proposal; changing a unit cost here does not rewrite lines already on an estimate.
...
- **Invoices** — draws and retainage with payment history, outstanding AR, PDF, a client share link, payment terms copied from company defaults (company admin can edit; totals and dates fill on the invoice), and the job owner as project manager with their profile phone
- **Accounting** — company admin and the Accounting seat get a company Profit and Loss in QuickBooks form, plus invoices, expenses, and payments. **Settings → QuickBooks** downloads a `.qwc` for QuickBooks Desktop: it posts **approved invoices that already have line items** onto the matching **Customer:Job** so nobody retypes quantities and rates. Expenses and payments still mark entered by hand. The connector needs [`20260825210000_qbwc.sql`](https://raw.githubusercontent.com/bradytrock/truss/main/supabase/migrations/20260825210000_qbwc.sql) (or a fresh bootstrap). Password setup that errors with **gen_salt** needs [`20260825220000_qbwc_pgcrypto.sql`](https://raw.githubusercontent.com/bradytrock/truss/main/supabase/migrations/20260825220000_qbwc_pgcrypto.sql).
- **Log expense / Log payment** — Create (+) in the top right, or Ask Truss with a photo attached. A photo is required every time, whether or not AI reads the receipt. Images stay on the record. `OPENAI_API_KEY` fills vendor, amount, date, and account from the photo (`ANTHROPIC_API_KEY` is an optional fallback for receipts only).
