...
-- ========== 20260830120000_returning_client_leads.sql ==========
...
-- ========== 20260831120000_teams_authenticated_grants.sql ==========
...
-- ========== 20260831130000_business_cards.sql ==========
...
-- ========== 20260831150000_card_slug_trigger_rights.sql ==========
...
alter table public.companies
  ...
  add column if not exists card_logo_storage_path text not null default '';

alter table public.team_members
  add column if not exists photo_url text not null default '',
  add column if not exists photo_storage_path text not null default '';
...
begin
  ...
  v_company_json := jsonb_build_object(
    ...
    'postalCode', coalesce(v_company.postal_code, ''),
    'logoUrl', coalesce(v_company.logo_url, ''),
    'logoStoragePath', coalesce(v_company.logo_storage_path, ''),
    'cardLogoUrl', coalesce(v_company.card_logo_url, ''),
    'cardLogoStoragePath', coalesce(v_company.card_logo_storage_path, ''),
    'googleReviewUrl', v_default_review,
    'paymentVenmo', coalesce(v_company.payment_venmo, ''),
    'paymentZelle', coalesce(v_company.payment_zelle, ''),
    'paymentCashapp', coalesce(v_company.payment_cashapp, ''),
  ...
  return jsonb_build_object(
    ...
    'person', jsonb_build_object(
      ...
      'phone', coalesce(v_staff.phone, ''),
      'photoUrl', coalesce(v_staff.photo_url, ''),
      'photoStoragePath', coalesce(v_staff.photo_storage_path, ''),
      'googleReviewUrl', v_review,
      'cardSlug', v_staff.card_slug
    )
  );
...
-- ========== 20260906180000_estimate_margin_percent.sql ==========
-- Whole-estimate margin markup on the included line sum before discount/tax.

alter table public.estimates
  add column if not exists margin_percent numeric(8, 3) not null default 0;

alter table public.estimates drop constraint if exists estimates_margin_percent_check;
alter table public.estimates
  add constraint estimates_margin_percent_check
...
begin
  ...
  if est.subtotal_override is not null then
    ...
    v_subtotal := round(v_line_subtotal * (1 + greatest(coalesce(est.margin_percent, 0), 0) / 100.0), 2);
  end if;
  ...
  if v_line_subtotal > 0 and (est.subtotal_override is not null or coalesce(est.margin_percent, 0) > 0) then
    v_taxable := round(v_subtotal * (v_taxable / v_line_subtotal), 2);
  elsif (est.subtotal_override is not null or coalesce(est.margin_percent, 0) > 0) and v_line_subtotal = 0 then
    v_taxable := v_subtotal;
  end if;
...
-- Email campaigns (20260914180000_email_campaigns.sql)
...
-- ========== 20260922190000_voice_agents.sql ==========
...
-- ========== 20260923200000_invoice_files.sql ==========
...
-- ========== 20260925120000_shared_estimate_files.sql ==========
