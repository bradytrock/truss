...
export function CrmProvider({ children }: { children: ReactNode }) {
  ...
  const sendTextMessage = useCallback(
    async (input: {
      ...
      if (data.mocked) {
        toast.message(
          "Texts are not connected on this host. The text is logged on the job. Add MYCRMSIM_LOCATION_ID on Vercel and point myCRMSIM’s workspace webhook at /api/messages/inbound?token=…",
        );
      } else {
  ...
  const notifyStaffByText = useCallback(async (member: StaffMember | undefined, content: string) => {
    ...
    try {
      ...
      // Lead still saved if the text provider is down.
    }
  }, []);
  ...
  const updateJob = useCallback(async (id: string, patch: Partial<Job>, options?: { skipAudit?: boolean }) => {
    ...
    if (error && isMissingJobCoords(error)) {
      apply();
      toast.message(missingStormMapMessage());
      return finish(true);
    }
