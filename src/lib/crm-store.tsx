...
import {
  newRealtorPortalToken,
  realtorPortalInviteExpiry,
  realtorPortalUrl,
...
async function insertCatalogRows(
  ...
  const attempts: Array<{
    ...
    missingLists?: boolean;
    missingDescription?: boolean;
  }> = [
    { rows },
    {
      rows: rows.map(({ description: _description, ...row }) => row),
    ...
    {
      rows: rows.map(({ description: _description, margin_percent: _margin, ...row }) => row),
      missingDescription: true,
      missingMargin: true,
    },
    {
    ...
    {
      rows: rows.map(({ description: _description, price_list_id: _list, ...row }) => row),
      missingDescription: true,
      missingLists: true,
    },
    {
    ...
    {
      rows: rows.map(({ description: _description, margin_percent: _margin, price_list_id: _list, ...row }) => row),
      missingDescription: true,
      missingMargin: true,
      missingLists: true,
    },
  ...
  for (const attempt of attempts) {
    ...
    if (!error) {
      if (attempt.missingDescription) toast.message(missingCatalogDescriptionMessage());
    ...
    lastError = error;
    if (!isMissingCatalogDescription(error) && !isMissingCatalogMargin(error) && !isMissingPriceLists(error)) break;
  }
...
type CrmContextValue = CrmState & {
  ...
  createRealtorPortalInvite: (input: {
    contactId: string;
  }) => Promise<{ url: string; token: string } | null>;
...
export function CrmProvider({ children }: { children: ReactNode }) {
    ...
    async (input: Omit<Contact, "id">) => {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          listing_watch_url: contact.listingWatchUrl || "",
          listing_watch_enabled: Boolean(contact.listingWatchEnabled),
        })
        .select("*")
        .single();
      if (error || !data) {
  ...
  const addEstimate = useCallback(
    async (input: {
      ...
      if (error && isMissingEstimateMargin(error)) {
        const { margin_percent: _margin, ...withoutMargin } = payload;
        const retry = await supabase.from("estimates").insert(withoutMargin).select("*").single();
        data = retry.data;
        error = retry.error;
        if (!error && data) toast.message(missingEstimateMarginMessage());
      }
      if (error && isMissingEstimateLumpSum(error)) {
        const { subtotal_override: _override, hide_line_prices: _hide, ...withoutLump } = payload;
        const retry = await supabase.from("estimates").insert(withoutLump).select("*").single();
        data = retry.data;
        error = retry.error;
  ...
  const updateEstimate = useCallback(async (id: string, patch: Partial<Estimate>, options?: { skipAudit?: boolean }) => {
    ...
    if (error) {
      ...
      if (isMissingEstimateMargin(error)) {
        apply();
        toast.message(missingEstimateMarginMessage());
        await finish();
        return;
      }
      if (isMissingEstimateLumpSum(error)) {
        apply();
        toast.message(missingEstimateLumpSumMessage());
        await finish();
        return;
      }
      ...
      if (isMissingPaperArchive(error) && patch.archivedAt !== undefined) {
        apply();
        toast.message(missingPaperArchiveMessage());
        await finish();
        return;
      }
      toast.error(error.message);
  ...
  const updateCatalogItem = useCallback(async (id: string, patch: Partial<CatalogItem>) => {
    ...
    if (error && isMissingCatalogDescription(error)) {
      const { description: _description, ...rest } = catalogPatch(next);
      const retry = await supabase.from("catalog_items").update(rest).eq("id", id);
      if (!retry.error) {
        toast.message(missingCatalogDescriptionMessage());
        apply();
        return;
      }
      toast.error(retry.error.message);
    ...
    async (rows: CatalogImportDraft[], priceListId?: string | null) => {
      ...
      for (const change of toUpdate) {
        const { error } = await supabase
          ...
          .eq("id", change.id);
        if (error) {
          if (isMissingCatalogDescription(error)) {
            const { description: _description, ...rest } = catalogPatch(change.patch);
  ...
  const createRealtorPortalInvite = useCallback(
    async (input: { contactId: string }) => {
      const contactId = input.contactId.trim();
      if (!contactId) {
        toast.error("Choose a referral partner before sending a realtor portal link.");
        return null;
      }
      const contact = state.contacts.find((item) => item.id === contactId);
      if (!contact) {
        toast.error("That contact isn’t in this book anymore.");
        return null;
      }
      if (!contact.isReferralPartner) {
        toast.error("Realtor portal invites are for referral partners only.");
        return null;
      }

      const token = newRealtorPortalToken();
      const expiresAt = realtorPortalInviteExpiry();
      const url = realtorPortalUrl(token, typeof window !== "undefined" ? window.location.origin : "");
      const supabase = maybeClient();
      if (!supabase) {
        toast.message("Realtor portal link ready locally. Connect Supabase to save invites.");
        return { url, token };
      }
      const { error } = await supabase.from("realtor_portal_invites").insert({
        company_id: user.companyId,
        contact_id: contactId,
        token,
        expires_at: expiresAt,
      ...
      if (error) {
        if (isMissingRealtorPortal(error)) {
          toast.message(missingRealtorPortalMessage());
          return { url, token };
        }
        toast.error(error.message);
        return null;
      }
      toast.success(`Realtor portal link ready for ${contact.name}`);
      return { url, token };
    },
    [state.contacts, user.companyId, user.staffId],
  );
  ...
  const deleteJobPhoto = useCallback(
    async (id: string) => {
      ...
      if (job?.primaryPhotoId === id) {
        const nextPrimary =
          livePhotos(state.photos, current.jobId).find((photo) => photo.id !== id)?.id ?? null;
        await updateJob(current.jobId, { primaryPhotoId: nextPrimary }, { skipAudit: true });
      }
  ...
  const persistCompany = useCallback(
    async (next: CompanySettings, quiet = false) => {
      ...
      const requestedSlug = next.slug?.trim() ?? "";
      if (requestedSlug && companySlugIsReserved(normalizeCompanySlug(requestedSlug))) {
        toast.error("That card URL is reserved for the app. Pick another.");
        return null;
      }
      ...
      const settings = withSyncedDefaultEstimateTerms({
        ...
        socialFacebook: next.socialFacebook?.trim() ?? "",
        socialInstagram: next.socialInstagram?.trim() ?? "",
        socialYoutube: next.socialYoutube?.trim() ?? "",
        socialLinkedin: next.socialLinkedin?.trim() ?? "",
      ...
      if (error && isMissingCompanySlug(error)) {
        ...
        data = retry.data;
        error = retry.error;
        if (!error) toast.message(missingBusinessCardsMessage());
      }
      if (error || !data) {
        const missingColumn =
          error?.message?.includes("schema cache") ||
        ...
        toast.error(
          isReservedCompanySlugError(error)
            ? "That card URL is reserved for the app. Pick another."
            : isDuplicateCardSlug(error)
              ? "That card URL is already in use by another company."
              : isCardSlugPrivilegeError(error)
                ? cardSlugPrivilegeMessage()
                : missingColumn
                ? "Run supabase/bootstrap.sql (or 20260831130000_business_cards.sql) in the SQL editor, then try again."
  ...
  const persistStaffFields = useCallback(
    async (
      ...
      if (error && isMissingCardSlug(error)) {
        const { card_slug: _cardSlug, ...withoutSlug } = payload;
      ...
      if (error) {
        toast.error("Could not save teammate", {
          description: isDuplicateStaffEmail(error)
            ...
            : isDuplicateCardSlug(error)
              ? "That card URL already belongs to someone on this company."
              : isCardSlugPrivilegeError(error)
                ? cardSlugPrivilegeMessage()
    ...
    async (input: {
      ...
      const member: StaffMember = {
        ...
        cardSlug: mintPersonCardSlug(
          name,
          "",
          state.staff.map((item) => item.cardSlug),
        ),
        locked: false,
        restricted: false,
        inviteToken: token,
  ...
  const updateStaffAccount = useCallback(
    async (
      ...
      const mintingOwnCard = Boolean(
        patch.cardSlug &&
          id === viewer?.id &&
          !state.staff.find((member) => member.id === id)?.cardSlug,
      );
      const onlyOwnProfile = Object.keys(patch).every(
        (key) => profileKeys.has(key) || (mintingOwnCard && key === "cardSlug"),
      );
        ...
        cardSlug:
          patch.cardSlug !== undefined
            ? mintPersonCardSlug(
                ...
                patch.cardSlug,
                state.staff.filter((member) => member.id !== id).map((member) => member.cardSlug),
              )
            : current.cardSlug ||
              mintPersonCardSlug(
                patch.name !== undefined ? patch.name.trim() : current.name,
