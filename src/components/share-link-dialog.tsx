...
type TextStatus = { configured: boolean; fromNumber: string; provider?: string };
...
export function ShareLinkDialog({
  ...
  async function handleText() {
    ...
    try {
      ...
      if (mocked) {
        toast.message(
          "Texts are not connected on this website. Add MYCRMSIM_LOCATION_ID on the host and redeploy, then point myCRMSIM’s workspace webhook at /api/messages/inbound.",
        );
      } else {
        toast.success(
          ready.length === 1
          ...
          <div className="grid gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {textStatus?.configured
                ? `Texts go out over ${textStatus.provider === "sendblue" ? "Sendblue" : "myCRMSIM"}${textStatus.fromNumber ? ` (${textStatus.fromNumber})` : ""}. `
                : "Texts need MYCRMSIM_LOCATION_ID on the host — until then Send text previews without delivering. "}
              {(() => {
                const fromLabel =
